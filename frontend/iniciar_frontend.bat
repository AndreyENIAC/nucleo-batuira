...
if errorlevel 1 (
    "%VPY%" -m http.server 5500 --bind 127.0.0.1
    set "CODIGO=!errorlevel!"
) else (
    powershell -NoProfile -ExecutionPolicy Bypass -Command "& '%VPY%' -m http.server 5500 --bind 127.0.0.1 2^>^&1 | Tee-Object -FilePath '%LOG%' -Append; exit $LASTEXITCODE"
    set "CODIGO=!errorlevel!"
)
